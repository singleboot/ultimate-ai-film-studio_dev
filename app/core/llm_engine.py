...
class LLMEngine:
    ...
    def copy_model_to_folder(self, source_path: str) -> Dict:
        ...
        try:
            ...
            def do_copy(src_path, dest_path, fn):
                try:
                    copied = 0
                    ...
                    with open(src_path, 'rb') as fsrc:
                        with open(dest_path, 'wb') as fdest:
                            while True:
                                ...
                                fdest.write(buf)
                                copied += len(buf)
                                if fn in self._downloads:
                                    self._downloads[fn]["downloaded_bytes"] = copied
                                    self._downloads[fn]["progress"] = int((copied / total_size) * 100)
                    if fn in self._downloads:
                        self._downloads[fn]["status"] = "completed"
                except Exception as e:
                    if fn in self._downloads:
                        self._downloads[fn]["status"] = "failed"
                        self._downloads[fn]["error"] = str(e)
            
            thread = threading.Thread(target=do_copy, args=(src, dest, filename), daemon=True)
            thread.start()
            return {"success": True, "message": "Copy started in background", "filename": filename}
        except Exception as e:
    ...
    def _gpu_available(self) -> bool:
        try:
            ...
            if result.returncode != 0:
                return False
            # Check if ggml-cuda.dll dependencies load successfully
            import ctypes
            from pathlib import Path
            dll_path = Path(__file__).parent.parent.parent / "bin" / "ggml-cuda.dll"
            if dll_path.exists():
                try:
    ...
    def launch_local_llm(self, provider_id: str, model_path: str = None) -> Dict:
        ...
        # Auto-resolve model_path if not provided
        if not model_path:
            model_path = self._get_provider_model(provider_id)
            if model_path == provider_id:
                model_path = ""
            if not model_path and provider_id == "app_llm":
                installed = self._get_app_llm_models()
                if installed:
                    model_path = installed[0]
            if not model_path:
                model_path = provider.get("default_model", "")
