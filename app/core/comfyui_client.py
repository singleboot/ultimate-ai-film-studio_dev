        ...
        try:
            ...
            for idx, nid in enumerate(slot_nodes):
                if input_images and idx < len(input_images):
                    ...
                    debug_log.append(f"Slot {nid} <- (empty, bypassing node)")
                    workflow[nid]["inputs"]["image_path"] = ""
                    # Direct bypass: Link the conditioning from Positive Ref/Negative Ref's source connection
                    # directly to the next block, bypassing this slot's ConditioningBypass execution.
                    # Find any ConditioningBypass that takes this slot as an input image:
                    bypass_target = f"20{idx}" if idx < 8 else None
                    if bypass_target and bypass_target in workflow:
                        # Reroute positive connection around the ConditioningBypass
                        conn = workflow[bypass_target]["inputs"]["conditioning"]
                        # Forward connection directly to the next Ref node
                        next_ref = f"20{idx + 1}" if idx < 7 else "60"
                        next_ref_key = "positive" if next_ref == "60" else "conditioning"
                        if next_ref in workflow:
                            workflow[next_ref]["inputs"][next_ref_key] = conn
                    
                    bypass_neg = f"30{idx}" if idx < 8 else None
                    if bypass_neg and bypass_neg in workflow:
                        conn_neg = workflow[bypass_neg]["inputs"]["conditioning"]
                        next_neg = f"30{idx + 1}" if idx < 7 else "60"
                        next_neg_key = "negative" if next_neg == "60" else "conditioning"
