...
if not exist "bin\llama-server.exe" (
    ...
    if exist "%TEMP%\llama.zip" (
        ...
        echo Searching for missing CUDA runtime DLLs (cublas, cudart) to enable GPU offloading...
        powershell -Command "$cuda_dlls = @('cublas64_*.dll', 'cublasLt64_*.dll', 'cudart64_*.dll'); $search_paths = @('C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v*\bin', '..\ultimate-ai-film-studio-v9\bin', '..\ultimate-ai-film-studio-v8\bin', 'comfyui\python_embeded\Lib\site-packages\torch\lib', 'F:\001 Comfyui Easy installer\ComfyUI-Easy-Install\ComfyUI-Easy-Install\python_embeded\Lib\site-packages\torch\lib'); foreach ($pattern in $cuda_dlls) { $found = $false; foreach ($path in $search_paths) { if (Test-Path $path) { $files = Get-ChildItem -Path $path -Filter $pattern -ErrorAction SilentlyContinue; if ($files) { foreach ($f in $files) { Copy-Item -Path $f.FullName -Destination 'bin\' -Force; Write-Host 'Copied' $f.Name 'from' $f.DirectoryName } $found = $true; break } } } if (-not $found) { Write-Host 'WARNING: Could not find' $pattern '. Llama-server will fall back to CPU if runtimes are missing.' }"
